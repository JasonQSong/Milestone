...
def getDelWord():
    noise_words_list = []
    with open('MilestonesA Del_YinYi_XiaoSong.txt', 'r', encoding='utf8') as file:
        while True:
...
def getRepWord():
    substitution_dic = {}
    with open('MilestonesA Sub_YinYi_XiaoSong.txt', 'r', encoding='utf8', errors='ignore') as file:
        while True:
            try:
                line = file.readline().strip()
                if line == '':
...
def filter_text(noise_words_list, substitution_dic):
    keywords_set = set()
    with open('MilestonesA Input_YinYi_XiaoSong.txt', 'r', encoding='utf8') as file:
        while True:
            line = file.readline()
            if line == '':
                break
            else:
                keywords_set |= separate_words(line,noise_words_list, substitution_dic)
...
<div style="margin-left: 200px; font-size: large"><table><tr><td style="padding-right: 50px">''')
        ...
        for i in sorted(keywords_set):
            ...
            file.write('''<li style="margin-bottom: 5px"><a href="#">'''+ i + '''</a></li>''')
            count +=1
